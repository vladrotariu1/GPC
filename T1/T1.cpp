...
void drawHexagon(float radius, float centerX, float centerY, float color[3]) {
   glColor3f(color[0], color[1], color[2]);
   glBegin(GL_POLYGON);
      for (int i = 0; i < 6; i++) {
         float angle = i * M_PI / 3;
         float x = centerX + radius * cos(angle);
         float y = centerX + radius * sin(angle);
...
void Display8() {
   ...
   float radiusMid = 0.32f;
   float radiusInside = 0.3f;

   float colorOutside[] = { 0.2, 0.15, 0.88 } ; // albastru
   float colorMid[] = { 1, 0.1, 0.1 } ; // alb
   float colorInside[] = { 1, 1, 1 } ; // alb

   drawHexagon(radiusOutside, centerX, centerY, colorOutside);
   drawHexagon(radiusMid, centerX, centerY, colorMid);
   drawHexagon(radiusInside, centerX, centerY, colorInside);
}
